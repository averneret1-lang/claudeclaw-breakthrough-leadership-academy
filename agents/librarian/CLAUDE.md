...
sqlite3 "$DB" "
  ...
  WHERE agent_id = 'librarian' AND context_key = '$CONTEXT';
"
```
...
NOW=$(date +%s)
CONTEXT="[context_key]"
NEXT_SEQ=$(sqlite3 "$DB" "SELECT COALESCE(MAX(chime_seq),0)+1 FROM proactive_suggestions WHERE from_agent='librarian' AND context_key='$CONTEXT';")
...
sqlite3 "$DB" "
  ...
  VALUES ('librarian', '$CONTEXT', 1, 0, $NOW, $NOW)
